...
jQuery(document).ready(function($){  
	...
	<!-- position 08 1 -->
		...
		$('.vt-slider8 div.products-grid').owlCarousel({
			items: 4,
			itemsCustom: [ 
				[0, 1], 
				[480, 2], 
			...
			afterAction: function (e) {
				if(this.$owlItems.length > this.options.items){
					$('.vt-slider8 .navslider').show();
				}else{
					$('.vt-slider8 .navslider').hide();
				}
			}
			//scrollPerPage: true,
		});
		$('.vt-slider8 .navslider .prev').on('click', function(e){
			e.preventDefault();
			$('.vt-slider8 div.products-grid').trigger('owl.prev');
		});
		$('.vt-slider8 .navslider .next').on('click', function(e){
			e.preventDefault();
